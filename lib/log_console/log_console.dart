...
class _LogConsoleState extends State<LogConsole> {
  ...
  void _refreshFilter() {
    var newFilteredBuffer = _renderedBuffer.where((it) {
      var logLevelMatches = it.level.levelIndex >= _filterLevel.levelIndex;
      if (!logLevelMatches) {
        return false;
  ...
  List<DropdownMenuItem<LevelBean>> getListData(){
    List<DropdownMenuItem<LevelBean>> items=new List();
    items.addAll(LogLevels.leves.map<DropdownMenuItem<LevelBean>>((bean){
      return  DropdownMenuItem<LevelBean>(
        child: Text(bean.levelType.toString()),
        value: bean,
      );
    }).toList());
    return items;
